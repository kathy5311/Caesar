...
"""
        ...
        #batch=1일 때 squeeze문제 방지하기 위해 batch=1일 땐 squeeze를 빼도록 함
        #print(G.to(device))
        pred = model(G.to(device)).squeeze()
        ...
        Ssum = torch.sum(torch.einsum('bij,i->bj',mask,pred),dim=1) # sum over all resno atoms
        #print(Ssum)
        #print(pred)
        #print()
        #print(Ssum.shape, label.shape)
        label = label.to(device)
        #label =torch.tensor(label, device=device)
        #label error로 텐서로 바꿈!
        loss1 = lossfunc(Ssum,label)
        ...
        if verbose:
            verbl = ''
            for tag,S,l in zip(info['target'], Ssum, label):
                verbl += f"{tag:9s} {float(S):5.3f} {float(l):5.3f} | "
                if torch.isnan(S).any() or torch.isnan(label).any():
                    print("S: ",torch.isnan(S).any())
                    print("label: ",torch.isnan(label).any())
                    print(tag)
