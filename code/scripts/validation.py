...
def run_an_epoch(model, data_loader, train): #verbose delete
    ...
    for i,(G,label,mask,info,label_int) in enumerate(data_loader):
        ...
        if type(G) != list:
            ...
            continue
"""
def expected_value(Ssum_soft):
    device = torch.device("cuda:0" if (torch.cuda.is_available()) else "cpu")
    #print(Ssum_soft)
    #print()
    range_tensor=torch.tensor([0.125,0.375,0.625,0.875,1.125,1.375,1.625,1.875,2.125,2.375,2.5])
    expected_value = torch.sum(Ssum_soft * range_tensor, dim=1)
    expected_value=round(expected_value.item(),4)
